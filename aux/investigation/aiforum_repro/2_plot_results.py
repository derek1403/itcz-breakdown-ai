...
def main():
    files = sorted(glob.glob("output/aiforum_*_amp*.h5"))
    if not files:
        print("no output/aiforum_*_amp*.h5 yet"); return
    ...
    for f in files:
        ...
        tag = os.path.basename(f).replace("aiforum_", "").replace(".h5", "")  # e.g. JAS_Deep_amp2.5
