"""Step 1 -- standard ITCZ breakdown (diabatic heating).

Heating ON for the first ``forcing_days`` (default 7 d), OFF afterwards; no locking.

    python scripts/run_step1.py [--model pangu --background JJA --amp_K 10 --heat_type Deep]
"""
import argparse
import os
...
def main():
    ...
    ap.add_argument("--amp_K", type=float, default=None, help="heating rate K/day")
    ap.add_argument("--heat_type", default=None, help="Deep|Stratiform|Shallow")
    ...
    cfg["experiment"] = {
        "name": f"step1_{cfg['model']}_{cfg['background']}_{ht}_{amp:g}Kday",
        "forcing_type": "heating", "persistent": False, "lock": "none", "seed_npz": None,
    }
    if args.exp:
        cfg["experiment"]["name"] = f"{os.path.basename(os.path.normpath(args.exp))}/step1"
        cfg["experiment"]["out_dir"] = cfgmod.step_out_dir(args.exp, 1)
