...
```說明
參考文獻（author–year 制，正文引用格式如 Hakim and Masanam 2024）。
每一筆都經過網路查證（期刊、卷期、DOI）。
AI-Forum 為實驗室內部報告（作者為本文共同作者），不需引用條目，正文亦不特別標註。
```
...
*Data/imagery:* Himawari full-disk imagery via the PCCU/ASRAD catalog
(https://asrad.pccu.edu.tw/); ERA5 via the Copernicus Climate Data Store.
